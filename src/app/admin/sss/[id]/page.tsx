...
export default function EditFAQPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const onSubmit = async (data: FAQFormData) => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Soru güncellenirken hata oluştu"
      );
    } finally {
      setSaving(false);
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="font-heading text-2xl font-bold text-primary">Soru Düzenle</h2>
            <p className="text-gray-500 font-body text-sm mt-1">Soruyu düzenleyin</p>
          </div>
        </div>
        <button
